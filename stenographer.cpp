    ...
    QObject(parent)
{
    ...
    connect(this->tts, SIGNAL(finished()), this, SLOT(ttsFinished()));
    ...
    QObject::connect(googleSpeech,SIGNAL(error()),this,SLOT(googleHasError()));
...
void Stenographer::addRecText(QString data)
{
    ...
    emit updateData(QVariant(QObject::tr(("User: " + data.split("\n").at(1)).toStdString().c_str())));
    if(this->m_isDialog){
        QString ans = this->answer->get(data.split("\n").at(1));
        QString disp_ans = ans;
        emit updateData(QVariant(QObject::tr(("PC: " + disp_ans.replace("+","").replace("=","")).toStdString().c_str())));
        if(!this->m_isTelling)
        {
            this->m_isTelling = true;
            this->tts->say(ans);
        }
//        this->beginStopRec();
    }
...
void Stenographer::ttsFinished()
{
    this->m_isTelling = false;
    this->beginStopRec();
    this->m_isActive = true;
}

void Stenographer::googleHasError()
{
    this->beginStopRec();
    this->m_isActive = true;
}
